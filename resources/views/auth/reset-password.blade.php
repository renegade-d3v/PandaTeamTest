<x-guest-layout>
    ...
    <div>
        <a href="/" class="hover:rotate-2">
            <x-application-logo class="w-20 h-20 fill-current" />
        </a>
    </div>
    <h2 class="mx-2 my-3 md:my-6 text-center text-3xl font-extrabold text-white">{{ __('Скидання пароля') }}</h2>
    <div class="w-full sm:max-w-md px-6 py-4 bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg">
        <form class="space-y-6 md:m-4" method="POST" action="{{ route('password.store') }}">
            @csrf
